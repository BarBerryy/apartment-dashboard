...
type BuildingClass = 'new_optimal' | 'osii_optimal' | 'market_trends' | 'comfort_plus' | 'comfort' | 'business';

interface ApartmentMixItem {
  type: string;
  percent: number;
  area: string;
  count: number;
...
interface DetailedLayoutItem {
  title: string;
  concept: string;
  apartments: ApartmentLayout[];
}

interface DetailedLayoutsMap {
  [key: string]: DetailedLayoutItem;
}

interface FinancialMetricsItem {
  pricePerSqm: number;
  avgUnitPrice: number;
  totalRevenue: number;
...
const detailedLayouts: DetailedLayoutsMap = {
  new_optimal: {
...
const financialMetrics: FinancialMetricsMap = {
  new_optimal: {
    pricePerSqm: 1750,
    avgUnitPrice: 121875,
...
function parseAreaRange(area: string): { min: number; max: number; avg: number } {
...
function getPercentageColorClass(percent: number): string {
  if (percent >= 25) {
  ...
  if (!currentLayout) {
    return (
      <div
        ...
        onClick={function() { setShowDetailModal(false); }}
      >
        <div
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Содержимое */}
        <div className="p-6">
          {currentLayout.apartments.map(function(apt: ApartmentLayout, index: number) {
            return (
              <div key={index} className="mb-8 last:mb-0">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse">
                    ...
                    <tbody>
                      {apt.rooms.map(function(room: RoomDetail, roomIndex: number) {
                        return (
...
const OptimizedApartmentMixDashboard: React.FC = () => {
  const [buildingClass, setBuildingClass] = useState<BuildingClass>('new_optimal');
  const [showDetailModal, setShowDetailModal] = useState<boolean>(false);

  const currentData: ApartmentDataItem = apartmentData[buildingClass];
  const currentMetrics: FinancialMetricsItem = financialMetrics[buildingClass];

  const areaRanges: AreaRangeItem[] = currentData.mix
    .filter(function(item: ApartmentMixItem) {
      return item.area !== '-';
    })
    .map(function(item: ApartmentMixItem) {
  ...
  const handleBuildingClassChange = function(newClass: BuildingClass): void {
    setBuildingClass(newClass);
  };

  const handleShowDetailModal = function(): void {
    setShowDetailModal(true);
  };

  const handleHideDetailModal = function(show: boolean): void {
    setShowDetailModal(show);
  };

  const isOptimalClass = buildingClass === 'new_optimal' || buildingClass === 'osii_optimal' || buildingClass === 'market_trends';
    ...
    <div className="w-full max-w-7xl mx-auto p-6 bg-gradient-to-br from-emerald-50 via-blue-50 to-purple-50">
      <div className="bg-white rounded-xl shadow-2xl p-8">
        ...
        <div className="mb-8 flex flex-wrap justify-center gap-4">
          <button
            onClick={function() { handleBuildingClassChange('new_optimal'); }}
            className={
              buildingClass === 'new_optimal'
                ? 'px-6 py-3 rounded-lg font-semibold transition-all bg-gradient-to-r from-emerald-600 to-green-600 text-white shadow-lg scale-105 ring-4 ring-emerald-200'
          ...
          <button
            onClick={function() { handleBuildingClassChange('osii_optimal'); }}
            className={
              buildingClass === 'osii_optimal'
                ? 'px-6 py-3 rounded-lg font-semibold transition-all bg-gradient-to-r from-teal-600 to-cyan-600 text-white shadow-lg scale-105 ring-4 ring-teal-200'
                : 'px-6 py-3 rounded-lg font-semibold transition-all bg-gray-200 text-gray-700 hover:bg-gray-300'
            }
          >
            Бойкент 💚
          ...
          <button
            onClick={function() { handleBuildingClassChange('market_trends'); }}
            className={
              buildingClass === 'market_trends'
                ? 'px-6 py-3 rounded-lg font-semibold transition-all bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-lg scale-105 ring-4 ring-purple-200'
                : 'px-6 py-3 rounded-lg font-semibold transition-all bg-gray-200 text-gray-700 hover:bg-gray-300'
            }
          >
            Бойкент (Тренды)
          ...
          <button
            onClick={function() { handleBuildingClassChange('comfort'); }}
            className={
              buildingClass === 'comfort'
                ? 'px-6 py-3 rounded-lg font-semibold transition-all bg-blue-600 text-white shadow-lg scale-105'
                : 'px-6 py-3 rounded-lg font-semibold transition-all bg-gray-200 text-gray-700 hover:bg-gray-300'
            }
          >
            Комфорт
          ...
          <button
            onClick={function() { handleBuildingClassChange('comfort_plus'); }}
            className={
          ...
          <button
            onClick={function() { handleBuildingClassChange('business'); }}
            className={
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg p-4 text-white">
            ...
            <div className="text-3xl font-bold">{formatNumber(currentData.totalArea)} м²</div>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-gray-50 rounded-lg p-6">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  data={currentData.mix.filter(function(item: ApartmentMixItem) { return item.count > 0; })}
                  dataKey="percent"
                  nameKey="type"
                  cx="50%"
                  cy="50%"
                  ...
                  startAngle={90}
                  endAngle={-270}
                  label={function(entry: { type: string; percent: number }) { return entry.type + ': ' + entry.percent + '%'; }}
                >
                  {currentData.mix
                    .filter(function(item: ApartmentMixItem) { return item.count > 0; })
                    .map(function(entry: ApartmentMixItem, index: number) {
        ...
        <div className="bg-gray-50 rounded-lg p-6 mb-8">
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              ...
              <tbody>
                {currentData.mix.map(function(item: ApartmentMixItem, index: number) {
                  const parsed = parseAreaRange(item.area);
                  const avgPrice = Math.round(parsed.avg * currentMetrics.pricePerSqm);
                  const totalRevenue = avgPrice * item.count;

                  return (
                    <tr key={index} className={item.count === 0 ? 'hover:bg-gray-100 opacity-50' : 'hover:bg-gray-100'}>
                      ...
                      <td className="border border-gray-300 px-4 py-3 text-center">
                        <span className={'inline-block px-3 py-1 rounded-full font-semibold ' + getPercentageColorClass(item.percent)}>
                          {item.percent}%
                        </span>
                      </td>
                      ...
                      <td className="border border-gray-300 px-4 py-3 text-right font-semibold text-green-600">
                        {item.count > 0 ? '$' + formatNumber(totalRevenue) : '-'}
                      </td>
                    </tr>
                  );
                })}
              ...
              <tfoot>
                <tr className="bg-gray-200 font-bold">
                  ...
                  <td className="border border-gray-300 px-4 py-3 text-right">
                    ${formatNumber(Math.round(currentMetrics.avgUnitPrice))}
                  </td>
                  <td className="border border-gray-300 px-4 py-3 text-right text-green-700">
                    ${formatNumber(currentMetrics.totalRevenue)}
                  </td>
                </tr>
              </tfoot>
            </table>
      ...
      <DetailModal
        ...
        setShowDetailModal={handleHideDetailModal}
        buildingClass={buildingClass}
